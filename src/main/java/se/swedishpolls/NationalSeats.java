...
public class NationalSeats {
  ...
  /**
   * One election's approved allocation rule. The first divisor is the era's: 1.4 for 2010 and 2014,
   * 1.2 from the change first applied in 2018.
   */
  public record Rules(
      int electionYear,
      int seats,
  ...
  public NationalSeats(JdbcClient db) {
    this.db = db;
  }

  /**
   * The stored rule of one election. An unlisted year has no configured rule; the latest row is
   * never reused for it, because the divisor a year uses is the divisor of that election.
   */
  public Rules rules(int electionYear) {
    return db.sql(
            """
            ...
            FROM national_allocation_rule WHERE election_year = :year
            """)
  ...
  public static boolean qualifies(double share, Rules rules) {
   ...
   */
  public static Allocation allocate(Map<String, Double> shares, Rules rules) {
   ...
   */
  public static SeatDraws allocateDraws(JointUncertainty.Draws draws, Rules rules) {
  ...
  public static Summary summarize(SeatDraws seatDraws, double intervalLevel) {
    ...
    final Rules rules = seatDraws.rules();
