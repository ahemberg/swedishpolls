...
public final class PollObservations {
    ...
    public record Observation(PollCsv.Poll poll, LocalDate midpoint, SimpleMatrix ilr,
                              SimpleMatrix covariance, int replacedZeros) {}
    public record Exclusion(int rowNumber, List<String> reasons) {
        public Exclusion { reasons = List.copyOf(reasons); }
    }
    public record Batch(Roster.CoveragePeriod period, List<String> components, SimpleMatrix basis,
                        List<Observation> observations, List<Exclusion> exclusions) {
        public Batch {
    ...
    public static Batch prepare(Roster.CoveragePeriod period, List<PollCsv.Poll> polls) {
        ...
        if (period.roster().size() != expected.size() || !expected.equals(new HashSet<>(period.roster())))
        ...
        for (var poll : polls) {
            ...
            if (!Double.isFinite(n) || n <= 0)
                throw new IllegalArgumentException("Unrepresentable sample size at row " + poll.rowNumber());
            ...
            for (int c = 0; c < size; c++) {
                ...
                if (!Double.isFinite(proportions[c]) || share.signum() < 0
            ...
            if (zeros > 0) {
                ...
                for (int c = 0; c < size; c++) proportions[c] = proportions[c] == 0
                        ? delta : proportions[c] * (1 - zeros * delta);
            }
            ...
            for (int r = 0; r < size - 1; r++) {
                for (int c = 0; c <= r; c++) {
                    double value = 0;
                    for (int j = 0; j < size; j++) value += basis.get(r, j) * basis.get(c, j)
                            / (n * proportions[j]);
                    if (!Double.isFinite(value))
                        throw new IllegalArgumentException("Nonfinite observation covariance at row " + poll.rowNumber());
            ...
            if (!DecompositionFactory_DDRM.chol(size - 1, true).decompose(covariance.getDDRM().copy()))
                throw new IllegalArgumentException("Observation covariance is not positive definite at row " + poll.rowNumber());
            var midpoint = poll.collectionFrom().plusDays(ChronoUnit.DAYS.between(poll.collectionFrom(), poll.collectionTo()) / 2);
