...
public final class PredictiveComparison {
    ...
    public record Result(double baselineDifference, double referenceDifference, double pairedStandardError) {
    ...
    public static Result evaluate(double[] candidate, double[] baseline, double[] reference) {
        if (candidate == null || baseline == null || reference == null || candidate.length < 8
                || baseline.length != candidate.length || reference.length != candidate.length) {
        ...
        for (int i = 0; i < count; i++) {
            if (!Double.isFinite(candidate[i]) || !Double.isFinite(baseline[i]) || !Double.isFinite(reference[i])) {
        ...
        for (int lag = 0; lag <= 3; lag++) {
            ...
            for (int i = lag; i < count; i++) {
                covariance += (differences[i] - referenceMean) * (differences[i - lag] - referenceMean) / count;
            }
            variance += (lag == 0 ? 1 : 2 * (1 - lag / 4.0)) * covariance / count;
        }
        if (!Double.isFinite(baselineMean) || !Double.isFinite(referenceMean)
                || !Double.isFinite(variance) || variance < 0) {
