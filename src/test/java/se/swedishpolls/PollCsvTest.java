  ...
  void rejectsIncompleteDocumentsAndQuarantinesDuplicateKeysIncludingNullPublication() {
    for (java.lang.String body :
        java.util.List.of(
            ...
            HEADER + "short,row\n",
            HEADER + "\"unterminated",
            HEADER.replace("Company", "house") + ROW))
      assertThrows(
        ...
        PollCsv.parse(
            csv(
                ROW.replace("Ipsos", "Demoskop").replace("2020", "2018")
                    + ROW.replace("Ipsos", "Demoskop")
                    + ROW.replace("Ipsos", "Inizio")));
  ...
  void agreesWithPinnedDataAuditWithoutRoundingOrRescaling() throws Exception {
    ...
    final java.util.List<se.swedishpolls.PollCsv.Poll> retained =
        rows.stream()
            .filter(
                p ->
                    p.collectionFrom() != null
                        && p.collectionTo() != null
                        && p.sampleSize() != null
                        && !p.collectionTo().isBefore(java.time.LocalDate.of(2006, 9, 1)))
            .toList();
    ...
    assertEquals(
        ...
        retained.stream()
            .filter(
                p ->
                    java.util.List.of("M", "L", "C", "KD", "S", "V", "MP", "SD").stream()
                        .anyMatch(
                            party ->
                                p.shares().get(party) != null && p.shares().get(party).scale() > 2))
            .count());
    assertEquals(
        5,
        retained.stream()
            .filter(p -> p.remainder() != null && p.remainder().signum() == 0)
            .count());
    assertTrue(
        retained.stream().noneMatch(p -> p.remainder() != null && p.remainder().signum() < 0));
    assertTrue(
        rows.stream()
            .filter(p -> p.surveyType().equals("exit_or_election_day"))
            .noneMatch(PollCsv.Poll::eligible));
