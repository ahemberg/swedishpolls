...
class DevelopmentTuningTest {
    ...
    private static final List<LocalDate> ELECTIONS = List.of(LocalDate.of(2014, 9, 14), LocalDate.of(2018, 9, 9));
    private static final DevelopmentTuning.Fold FOLD =
            new DevelopmentTuning.Fold(LocalDate.of(2014, 6, 30), LocalDate.of(2014, 8, 4));
    private static final DevelopmentTuning.Grid GRID = new DevelopmentTuning.Grid(
            List.of(1e-5, 1e-4, 1e-3), List.of(0.05, 0.2), List.of(1.0, 2.0));

    /** One eligible poll, published on its collection end unless the caller overrides the publication date. */
    private static String row(String institute, int from, int to, String m, String publication) {
        return "2014-01," + institute + "," + m + ",5,8,5,25,8,5,12,1,10,1000,"
                + (publication == null ? START.plusDays(to).toString() : publication) + "," + institute + ","
                + START.plusDays(from) + "," + START.plusDays(to) + ",FALSE\n";
    }
    ...
    private static Roster.CoveragePeriod period(boolean fi) {
        return new Roster.CoveragePeriod("test", START, START.plusDays(3650),
    ...
    void gridAxesMustBeAscendingAndAdmissibleAndExpandInTieBreakingOrder() {
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(), List.of(0.05), List.of(1.0)));
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(1e-3, 1e-4), List.of(0.05), List.of(1.0)));
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(1e-4, 1e-4), List.of(0.05), List.of(1.0)));
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(1e-4), List.of(-0.05), List.of(1.0)));
        // A static opinion state is an admissible walk variance; a zero house scale or multiplier is not.
        assertDoesNotThrow(() -> new DevelopmentTuning.Grid(List.of(0.0, 1e-4), List.of(0.05), List.of(1.0)));
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(1e-4), List.of(0.0), List.of(1.0)));
        assertThrows(IllegalArgumentException.class, () -> new DevelopmentTuning.Grid(List.of(1e-4), List.of(0.05), List.of(0.0)));
    ...
    void trainingKeepsOnlyEligiblePollsPublishedByTheCutoff() {
        var rows = row("Novus", 0, 2, "20")
                + row("Sifo", 200, 202, "21")                              // published after the cutoff
                + row("Ipsos", 3, 5, "22", "NA")                           // unknown publication date
                + row("Demoskop", 6, 8, "22", START.plusDays(200).toString())  // late publication of early fieldwork
        ...
        assertTrue(training.stream().allMatch(poll -> !poll.publicationDate().isAfter(FOLD.cutoff())
    ...
    void tuningResolvesTheGridMaximumAndCountsItsTrainingInputsForBothRosters() {
        ...
        for (boolean fi : List.of(false, true)) {
            ...
            // The resolved point is the grid maximum of the same plug-in marginal likelihood, recomputed directly.
    ...
    void gridBoundariesAreRecordedOnEveryAxisWhoseOptimumSitsAtAnEnd() {
        ...
        assertEquals(List.of("walkVariance:lower", "walkVariance:upper", "houseScale:lower", "houseScale:upper",
                        "covarianceMultiplier:lower", "covarianceMultiplier:upper"),
                DevelopmentTuning.tune(period(false), polls, ELECTIONS, FOLD, fixed).gridBoundaries());

        // Two steadily disagreeing institutes need a house scale above 1e-9, so that axis hits its upper end.
        var narrow = new DevelopmentTuning.Grid(List.of(1e-9, 1e-8), List.of(1e-9, 1e-8), List.of(1.0, 1e6));
    ...
    void anExactTieKeepsTheSmallestParametersInGridOrder() {
        // A single poll on the period start day never applies the daily walk, so every walk variance ties exactly.
        var resolved = DevelopmentTuning.tune(period(false), polls(row("Novus", 0, 0, "20")), ELECTIONS, FOLD, GRID);
    ...
    void tuneAllListsUnresolvedFoldsAndBlocksTheGateWithoutLosingTheOtherFolds() {
        ...
        var tuning = DevelopmentTuning.tuneAll(List.of(period(false), period(true)), polls, ELECTIONS, protocol);
        ...
        assertTrue(tuning.unresolved().stream().allMatch(u -> u.fold().equals(late)
                && u.reason().contains("no eligible training observation")));
        assertTrue(tuning.gate().blocked());
        assertTrue(tuning.gate().reasons().stream().anyMatch(reason -> reason.contains("did not resolve")),
                () -> tuning.gate().reasons().toString());

        // A failed fit is listed with its reason too, rather than stopping the periods and folds after it.
        var overflowing = new DevelopmentTuning.Grid(List.of(Double.MAX_VALUE), List.of(0.05), List.of(1.0));
        var failing = DevelopmentTuning.tuneAll(List.of(period(false)), polls, ELECTIONS,
        ...
        assertTrue(failing.unresolved().getFirst().reason().contains("Failed fit"),
    ...
    void emptyFoldsAndFailedFitsStopTuningInsteadOfBeingDroppedFromTheGrid() {
        ...
        // The walk overflows the opinion covariance before the poll, which the fit rejects rather than clipping.
        var overflowing = new DevelopmentTuning.Grid(List.of(1e-4, Double.MAX_VALUE), List.of(0.05), List.of(1.0));
        var failed = assertThrows(IllegalArgumentException.class,
    ...
    void theRegisteredProtocolFreezesTheFoldsAndTheCandidateGrid() {
        ...
        assertEquals(new DevelopmentTuning.Fold(LocalDate.of(2014, 1, 15), LocalDate.of(2014, 2, 19)),
                protocol.folds().getFirst());
        assertEquals(LocalDate.of(2021, 10, 5), protocol.folds().getLast().cutoff());
        assertTrue(protocol.folds().stream().allMatch(fold ->
                fold.scoreThrough().equals(fold.cutoff().plusDays(35))), "Folds score 35 days ahead");
