...
class ElectionReferenceIT {
    ...
    void migrationPersistsOfficialReferencesAndRulesWithoutCreatingPollObservations() {
        ...
        try {
            ...
            assertEquals(List.of("2010-09-19", "2014-09-14", "2018-09-09", "2022-09-11"),
                    db.sql("SELECT election_date::text FROM election_reference ORDER BY election_date").query(String.class).list());
            assertEquals(List.of(5960408, 6231573, 6476725, 6477970),
                    db.sql("SELECT valid_votes FROM election_reference ORDER BY election_date").query(Integer.class).list());
            assertEquals(40, db.sql("SELECT count(*) FROM election_party_reference").query(Integer.class).single());
            ...
            // Official counts distinguish the equal rounded 2010 V/KD shares and retain historical FP as L.
            assertEquals(List.of(333696, 334053), db.sql("""
            ...
            assertEquals(List.of(24, 0, 19, 16, 68, 18, 0, 107, 73, 24), db.sql("""
                    SELECT official_seats FROM election_party_reference WHERE election_date = '2022-09-11' ORDER BY component
                    """).query(Integer.class).list());

            assertEquals(List.of(2010, 2014, 2018, 2022, 2026), db.sql("""
                    SELECT election_year FROM national_allocation_rule ORDER BY election_year
                    """).query(Integer.class).list());
            assertEquals(List.of(new BigDecimal("1.4"), new BigDecimal("1.4"), new BigDecimal("1.2"),
                    new BigDecimal("1.2"), new BigDecimal("1.2")), db.sql("""
                    SELECT first_divisor FROM national_allocation_rule ORDER BY election_year
                    """).query(BigDecimal.class).list());
            assertEquals(5, db.sql("""
