...
class PollObservationsTest {
    ...
    private static Roster.CoveragePeriod period(boolean fi) {
        return new Roster.CoveragePeriod(fi ? "fi_candidate" : "eight", LocalDate.of(2014, 4, 9),
                LocalDate.of(2018, 9, 7), fi ? Stream.concat(PollCsv.PARTIES.stream(), Stream.of("FI")).toList()
                        : PollCsv.PARTIES, fi, !fi, "https://example.invalid/decision");
    }

    @Test
    void replacesOnlyExactZerosAndPreservesPositiveRatiosAndSourceValues() {
        var row = ROW.replace(",20,5,", ",0,5,").replace(",17,1,10,", ",17,0,10,");
        for (boolean fi : List.of(false, true)) {
            var polls = PollCsv.parse(PollCsvTest.csv(row));
            var batch = PollObservations.prepare(period(fi), polls);
            var observation = batch.observations().getFirst();
            assertEquals(fi ? 2 : 1, observation.replacedZeros());
            var p = proportions(batch);
        ...
        var capped = PollObservations.prepare(period(true), PollCsv.parse(PollCsvTest.csv(row.replace(",1000,", ",1,"))));
    ...
    void keepsOneObservationPerPollAndRecordsExclusionsWithoutChangingRoster() {
        var rows = ROW.replace("2016-06-19", "2016-06-02")
                + ROW.replace("Ipsos", "Novus").replace("2016-06-19", "2016-06-01").replace("2016-06-20", "NA")
        ...
        var polls = PollCsv.parse(PollCsvTest.csv(rows));
        var batch = PollObservations.prepare(period(true), polls);
        assertEquals(List.of(1, 2), batch.observations().stream().map(o -> o.poll().rowNumber()).toList());
        assertEquals(List.of(LocalDate.of(2016, 6, 1), LocalDate.of(2016, 6, 1)),
        ...
        assertEquals(List.of("outside_coverage_period:fi_candidate"), batch.exclusions().get(5).reasons());
        assertTrue(batch.exclusions().get(6).reasons().contains("reversed_collection_dates"));
        var eight = PollObservations.prepare(period(false), polls);
        assertEquals(List.of(1, 2, 3, 7), eight.observations().stream().map(o -> o.poll().rowNumber()).toList());
    }

    @Test
    void rejectsMalformedRostersAndUnrepresentableNumericalInputs() {
        var polls = PollCsv.parse(PollCsvTest.csv(ROW));
        for (var roster : List.of(List.of("M"), Stream.concat(PollCsv.PARTIES.stream(), Stream.of("M")).toList())) {
            var invalid = new Roster.CoveragePeriod("bad", period(false).effectiveFrom(), period(false).effectiveTo(),
                    roster, false, false, "https://example.invalid");
            assertThrows(IllegalArgumentException.class, () -> PollObservations.prepare(invalid, polls));
        }
        // These remain valid decimal source values; double underflow must not turn a positive share into a valid zero.
        for (var row : List.of(ROW.replace(",20,5,", ",1e-999,5,"), ROW.replace(",1000,", ",1e999,")))
            assertThrows(IllegalArgumentException.class, () -> PollObservations.prepare(period(false), PollCsv.parse(PollCsvTest.csv(row))));
    }

    @Test
    void handlesZeroRemaindersWithoutClippingSmallPositiveSharesAndFollowsDeclaredOrder() {
        for (boolean fi : List.of(false, true)) {
            var zeroRemainder = ROW.replace(",20,5,", fi ? ",21,5," : ",22,5,");
            var batch = PollObservations.prepare(period(fi), PollCsv.parse(PollCsvTest.csv(zeroRemainder)));
        ...
        var small = PollObservations.prepare(period(false), PollCsv.parse(PollCsvTest.csv(ROW.replace(",20,5,", ",0.000001,5,"))));
        assertEquals(0, small.observations().getFirst().replacedZeros());
        assertEquals(1e-8, proportions(small).get(0), 1e-20);
        var reverse = new Roster.CoveragePeriod("reverse", period(false).effectiveFrom(), period(false).effectiveTo(),
                PollCsv.PARTIES.reversed(), false, true, "https://example.invalid");
        var reordered = PollObservations.prepare(reverse, PollCsv.parse(PollCsvTest.csv(ROW)));
    ...
    private static void checkGeometryAndDenseCovariance(PollObservations.Batch batch, SimpleMatrix p, double n) {
        var h = batch.basis();
        assertTrue(h.mult(h.transpose()).minus(SimpleMatrix.identity(h.getNumRows())).normF() < 1e-14);
        var ones = new SimpleMatrix(h.getNumCols(), 1);
        ones.fill(1);
        assertTrue(h.mult(ones).normF() < 1e-14);
        // Unsimplified delta method: J Cov(p) J', including all negative multinomial cross-terms.
        var diagonal = new SimpleMatrix(p.getNumRows(), p.getNumRows());
        var inverse = diagonal.copy();
        for (int i = 0; i < p.getNumRows(); i++) {
            diagonal.set(i, i, p.get(i));
            inverse.set(i, i, 1 / p.get(i));
        }
        var jacobian = h.mult(inverse);
        var reference = jacobian.mult(diagonal.minus(p.mult(p.transpose())).divide(n)).mult(jacobian.transpose());
        var actual = batch.observations().getFirst().covariance();
        assertTrue(actual.minus(reference).normF() < 1e-12);
        assertEquals(0, actual.minus(actual.transpose()).normF());
        assertTrue(DecompositionFactory_DDRM.chol(actual.getNumRows(), true).decompose(actual.getDDRM().copy()));
    }

    @Test
    void convertsBothRostersAtTheMidpointWithReferenceIlrAndFullCovariance() {
        // Independent 50-digit Decimal log/sqrt calculations for the stated percentage composition, n=1000.
        double[][] expected = {
            {0.9802581434685472, 0.18219594670412284, 0.5358670722168778, -1.1875175397137705,
                    0.2369884708195098, 0.6354306213564618, -0.5944380354331128, 1.4934286579968497},
            {0.9802581434685472, 0.18219594670412284, 0.5358670722168778, -1.1875175397137705,
                    0.2369884708195098, 0.6354306213564618, -0.5944380354331128, 2.146934086975881, 1.9202762246758065}
        };
        for (boolean fi : List.of(false, true)) {
            var polls = PollCsv.parse(PollCsvTest.csv(ROW));
            var batch = PollObservations.prepare(period(fi), polls);
            ...
            int last = covariance.getNumRows() - 1;
            assertEquals(fi ? 0.09221350762527233 : 0.04582244008714597, covariance.get(last, last), 1e-15);
            assertEquals(fi ? -0.008705563128087417 : 0.0008213933808860658, covariance.get(last, last - 1), 1e-15);
