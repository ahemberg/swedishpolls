...
class NationalSeatsTest {
  ...
  void exactlyFourPercentQualifiesAndAnUnroundedShareBelowItDoesNot() {
    final NationalAllocationRule rules = rules(2026, 1.2);
  ...
  void anExactTieGoesToTheEarlierPartyOfTheFixedOrderAndIsCounted() {
    final NationalAllocationRule order = small(List.of("A", "B"), 3);
    final NationalSeats.Allocation allocation =
    ...
    final NationalAllocationRule reversed = small(List.of("B", "A"), 3);
    assertEquals(
        Map.of("B", 2, "A", 1),
  ...
  void theFirstDivisorAppliesOnlyToAPartyWithNoSeatYet() {
    final NationalAllocationRule modified = small(List.of("A", "B"), 2);
    assertEquals(
        Map.of("A", 2, "B", 0),
        NationalSeats.allocate(Map.of("A", 70.0, "B", 25.0), modified).seats());
    final NationalAllocationRule unmodified =
  ...
  private static NationalSeats.SeatDraws draws(
      NationalAllocationRule rules, double[][] transformed) {
  ...
  static NationalAllocationRule rules(int electionYear, double firstDivisor) {
    return seats(electionYear, firstDivisor, 349);
  }

  private static NationalAllocationRule seats(int electionYear, double firstDivisor, int seats) {
  ...
  private static NationalAllocationRule small(List<String> tieOrder, int seats) {
    return new NationalAllocationRule(
        2026,
        seats,
        4,
