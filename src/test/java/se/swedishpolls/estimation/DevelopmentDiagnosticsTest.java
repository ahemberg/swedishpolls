...
class DevelopmentDiagnosticsTest {
  ...
  void scoresEveryEligiblePollPublishedInsideTheHorizonAndNoOther() {
    ...
    final se.swedishpolls.estimation.DevelopmentTuning.Fold fold =
  ...
  void theBaselineAveragesItsWindowAndFallsBackToTheFiveMostRecentWhenItIsThin() {
    final se.swedishpolls.estimation.PollObservations.Batch wide =
        batch(
            row("Novus", 0, 2, 3, "20")
                + row("Sifo", 30, 32, 33, "22")
                + row("Novus", 60, 62, 63, "24")
                + row("Sifo", 90, 92, 93, "26"));
    final se.swedishpolls.estimation.RecencyBaseline.Fit fit =
        RecencyBaseline.fit(wide, START.plusDays(93));
    ...
    final se.swedishpolls.estimation.PollObservations.Batch thin =
        batch(
            row("Novus", 0, 2, 3, "20")
                + row("Sifo", 10, 12, 13, "22")
                + row("Novus", 20, 22, 23, "24")
                + row("Sifo", 300, 302, 303, "26"));
    final se.swedishpolls.estimation.RecencyBaseline.Fit fallback =
        RecencyBaseline.fit(thin, START.plusDays(303));
    assertEquals(4, fallback.used().size());
    final se.swedishpolls.estimation.ModelValues predictive =
  ...
  void aFoldScoresAllThreeCandidatesOnTheSameHeldOutPollsAndRecordsWhatItRead() {
    ...
    final se.swedishpolls.estimation.DevelopmentTuning.Grid grid =
    ...
    for (java.util.List<se.swedishpolls.estimation.DevelopmentDiagnostics.Scored> scored :
  ...
  void coverageAndMisfitAreCountedPerPartyInstituteAndFieldworkBand() {
    ...
    final java.util.List<se.swedishpolls.estimation.DevelopmentDiagnostics.Misfit> misfit =
    ...
    for (se.swedishpolls.estimation.DevelopmentDiagnostics.Misfit row : misfit)
      assertTrue(
  ...
  void pollCountCenteringMovesTheReferenceWithoutMovingTheFit() {
    ...
    final se.swedishpolls.estimation.PollObservations.Batch batch = batch(rows.toString());
    assertEquals(
    ...
    final se.swedishpolls.estimation.DailyStateSpace.Fit equal =
        DailyStateSpace.fit(batch, ELECTIONS, PARAMETERS);
    final se.swedishpolls.estimation.DailyStateSpace.Fit counted =
        DailyStateSpace.fit(batch, ELECTIONS, PARAMETERS, DailyStateSpace.Centering.POLL_COUNT);
