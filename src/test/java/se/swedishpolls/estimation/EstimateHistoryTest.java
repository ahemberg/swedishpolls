...
class EstimateHistoryTest {
  ...
  void retainsEveryDayBetweenPollsAndStopsAtTheLastMidpointWithoutProjecting() {
    ...
    final se.swedishpolls.estimation.EstimateHistory.Estimated estimated =
        EstimateHistory.estimate(period, polls, ELECTIONS, POINT, rules(), DRAWS);

    assertEquals(1, estimated.segments().size());
    final se.swedishpolls.estimation.EstimateHistory.Segment segment =
        estimated.segments().getFirst();
    ...
    for (int day = 0; day < segment.days().size(); day++) {
      final se.swedishpolls.estimation.EstimateHistory.Day estimate = segment.days().get(day);
  ...
  void anOverlongGapEndsSupportInsteadOfBeingBridged() {
    ...
    assertEquals(
        ...
        estimated.boundaries().stream().map(EstimateHistory.Boundary::kind).toList());
    final se.swedishpolls.estimation.EstimateHistory.Boundary gap = estimated.boundaries().get(1);
  ...
  void theHeadlineIsDatedAtTheLastFieldworkDateAndCarriesTheLastEstimatedDay() {
    ...
    final se.swedishpolls.estimation.EstimateHistory.History history =
        history(
            List.of(period),
            polls,
            ELECTIONS,
    ...
    final se.swedishpolls.estimation.EstimateHistory.Day last =
        history.segments().getLast().days().getLast();
  ...
  void changesStayInsideOneSegmentAndAreSuppressedAcrossABoundary() {
    ...
    final se.swedishpolls.estimation.EstimateHistory.Change inside =
    ...
    final se.swedishpolls.estimation.EstimateHistory.Change across =
    ...
    final se.swedishpolls.estimation.EstimateHistory.Change before =
    ...
    final se.swedishpolls.estimation.EstimateHistory.Change unsupported =
  ...
  void filteredPublicationTimeStatesStayOutOfThePublishedHistory() {
    ...
    final java.util.List<se.swedishpolls.estimation.EstimateHistory.Composition> filtered =
        EstimateHistory.internalFiltered(period, polls, ELECTIONS, POINT, rules());
    final java.util.List<se.swedishpolls.estimation.EstimateHistory.Day> smoothed =
  ...
  void publishesTheDrawnMeanAndKeepsTheTransformOfTheMeanStateAsADiagnostic() {
    ...
    final se.swedishpolls.estimation.EstimateHistory.Estimated estimated =
        EstimateHistory.estimate(period, polls, ELECTIONS, POINT, rules(), DRAWS);
    final java.util.List<se.swedishpolls.estimation.JointUncertainty.Day> drawn =
        JointUncertainty.estimate(period, polls, ELECTIONS, POINT, rules(), DRAWS)
            .segments()
            .getFirst()
            .days();
    final java.util.List<se.swedishpolls.estimation.EstimateHistory.Composition> stateMean =
    ...
    for (int day = 0; day < days.size(); day++) {
      final se.swedishpolls.estimation.EstimateHistory.Day published = days.get(day);
      final se.swedishpolls.estimation.JointUncertainty.Day summary = drawn.get(day);
      assertEquals(summary.date(), published.date());
      // The same draws behind the published mean are behind its published endpoints.
      for (se.swedishpolls.estimation.JointUncertainty.Component component : summary.components()) {
        final se.swedishpolls.estimation.EstimateHistory.Estimate estimate =
  ...
  void aCycleWhoseInstituteEnsembleChangesIsMarkedAsAReferenceBreak() {
    ...
    assertEquals(1, estimated.segments().size());
    final java.util.List<se.swedishpolls.estimation.EstimateHistory.Boundary> reference =
  ...
  void aPeriodWithoutTunedParametersOrRecordedSupportIsRejected() {
    ...
    final se.swedishpolls.estimation.CoverageValidation.Validated unsupported =
