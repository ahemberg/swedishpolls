...
class WindowFilterTest {
  ...
  void fieldworkWindowsAndTheirPredictionsMatchTheDenseJointSystem() {
    for (se.swedishpolls.estimation.WindowFilter.Convention convention :
        WindowFilter.Convention.values())
      for (boolean fi : List.of(false, true)) {
        ...
        final java.util.List<se.swedishpolls.estimation.PollObservations.Observation> heldOut =
            batch(
                    fi,
                    200,
                    row("Novus", 40, 44, "26")
                        ...
                        + row("Skop", 130, 133, "23"))
                .observations();
        final se.swedishpolls.estimation.WindowFilter.Scored scored =
            WindowFilter.score(training, heldOut, ELECTIONS, PARAMETERS, convention);
        final se.swedishpolls.estimation.WindowFilterTest.Dense dense =
            dense(training, heldOut, convention);
  ...
  void everyPredictionConditionsOnTheWholeTrainingSetWhateverOrderTheWindowsClose() {
    ...
    final java.util.List<se.swedishpolls.estimation.PollObservations.Observation> early =
    ...
    final se.swedishpolls.estimation.PollObservations.Batch withoutLate =
        batch(false, 200, row("Sifo", 0, 2, "22"));
    final se.swedishpolls.estimation.WindowFilter.Scored ignoringLate =
        WindowFilter.score(
  ...
  void rejectsInadmissibleParametersAndEmptyTraining() {
    ...
    // observation to fit.
    final se.swedishpolls.estimation.PollObservations.Batch empty =
        batch(false, 5, row("Sifo", 50, 52, "22"));
    assertThrows(
        IllegalArgumentException.class,
        () ->
            WindowFilter.logLikelihood(
  ...
  private static Dense dense(
    ...
    final java.util.List<se.swedishpolls.estimation.PollObservations.Observation> trained =
        training.observations();
  ...
  private static SimpleMatrix block(
      ...
      WindowFilter.Convention convention,
      int dimension) {
    final se.swedishpolls.estimation.WindowFilter.Window left = convention.window(first);
    final se.swedishpolls.estimation.WindowFilter.Window right = convention.window(second);
    double covariance = 0;
