...
import { LEFT, PLOT_WIDTH } from '../src/chart.ts';
import {
  MINIMUM_SPAN,
  markerSpan,
  sourceMarks,
...
test('a marker spans its own interview dates rather than a sampled estimate day', () => {
  const span = markerSpan(observation(), YEAR);
...
test('a one-day period keeps a selectable width inside the plot', () => {
  const span = markerSpan(observation({ from: '2025-06-01', to: '2025-06-01' }), YEAR);
...
test('a period crossing either boundary is clipped to the plot and says so', () => {
  const early = markerSpan(observation({ from: '2024-12-20', to: '2025-01-05' }), YEAR);
  assert.equal(early.x1, LEFT);
  assert.equal(early.clippedFrom, true);
  assert.equal(early.clippedTo, false);
  const late = markerSpan(observation({ from: '2025-12-28', to: '2026-01-04' }), YEAR);
...
test('a single-day window still places its markers instead of dividing by nothing', () => {
  ...
  const span = markerSpan(observation({ from: '2025-06-01', to: '2025-06-01' }), point);
...
test('a missing share draws no marker and the remainder is never one', () => {
  const marks = sourceMarks([observation()], YEAR, ['S', 'M', 'FI']);
  assert.deepEqual(marks.map((mark) => mark.component), ['S', 'M']);
  assert.deepEqual(sourceMarks([observation()], YEAR, ['OTHER']), []);
});

test('marks carry the observation they came from so details stay complete', () => {
  const [mark] = sourceMarks([observation({ approximatePeriod: true })], YEAR, ['S']);
...
test('the axis reaches the highest reported share of the drawn parties alone', () => {
  ...
  assert.equal(sourceMaximum(polls, ['M']), 20);
  assert.equal(sourceMaximum(polls, ['S', 'M']), 45);
  assert.equal(sourceMaximum([], ['S']), 10);
});

test('year rules fall on every January inside the window, never on its first day', () => {
  assert.deepEqual(yearTicks(YEAR), []);
  assert.deepEqual(
    yearTicks({ id: 'all', from: '2023-06-01', to: '2025-03-01' }).map((tick) => tick.year),
    ['2024', '2025'],
  );
});
