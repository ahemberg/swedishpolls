...
assert.equal(page.eval("[...selected].join(',')"),'S,M');
...
assert.equal(empty.eval('selected.size'),0);
assert.equal(empty.elements.from.value,'2025-01-01');
assert.equal(empty.elements.variantName.textContent,'B · Sidopanel');
empty.eval("cycle(1)");assert.equal(empty.eval('variant'),'C');
