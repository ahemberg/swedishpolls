...
public publication: a set of immutable documents and a pinned source snapshot, served through the
[frozen v1 API contract](api-contract.md).
...
5. Write the candidate's `model_run`, `publication` and document rows.
6. Commit the `published` state and the current pointer together.

Any failure abandons the candidate, deletes its documents, records a `failed` attempt with the
cause chain, and marks the current pointer stale. The previous publication's rows are untouched.
...
Current responses carry `max-age=300`; permanent publication links carry
...
```sh
./mvnw -Dtest='ModelFreezeTest,PollQueryTest,EstimateQueryTest,TranslationsTest' test
